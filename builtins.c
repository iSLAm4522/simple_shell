...
void free_my_environ(shell_context_t *ctx)
{
	...
	if (ctx->my_environ)
	{
		for (i = 0; ctx->my_environ[i]; i++)
			free(ctx->my_environ[i]);
		free(ctx->my_environ);
		ctx->my_environ = NULL;
	}
}
...
int builtin_setenv(shell_context_t *ctx, char **args, char *input)
{
	...
	if (!ctx->my_environ)
	{
		ctx->my_environ = copy_environ();
		if (!ctx->my_environ)
		{
			fprintf(stderr, "%s: 1: setenv: allocation failed\n", ctx->program_name);
			return (1);
		}
		environ = ctx->my_environ;
	}
	var_len = string_length(args[1]);
	count = 0;
	while (ctx->my_environ[count])
	{
		if (string_length(ctx->my_environ[count]) > var_len &&
			ctx->my_environ[count][var_len] == '=')
		{
			int j, match = 1;

			for (j = 0; j < var_len; j++)
			{
				if (ctx->my_environ[count][j] != args[1][j])
				{
					match = 0;
					break;
				}
	...
	if (found_idx >= 0)
	{
		free(ctx->my_environ[found_idx]);
		ctx->my_environ[found_idx] = new_var;
		return (0);
	}
	...
	for (i = 0; i < count; i++)
		new_environ[i] = ctx->my_environ[i];
	new_environ[count] = new_var;
	new_environ[count + 1] = NULL;
	free(ctx->my_environ);
	ctx->my_environ = new_environ;
	environ = ctx->my_environ;
	return (0);
}
...
int builtin_unsetenv(shell_context_t *ctx, char **args, char *input)
{
	...
	while (ctx->my_environ[count])
	{
			...
			ctx->my_environ[count][var_len] == '=')
		{
			...
			for (k = 0; k < var_len; k++)
			{
				if (ctx->my_environ[count][k] != args[1][k])
				{
					match = 0;
					break;
				}
	...
	{
		if (i != found_idx)
			new_environ[j++] = ctx->my_environ[i];
		else
			free(ctx->my_environ[i]);
	}
	new_environ[j] = NULL;
...
* @ctx: Pointer to shell context (unused)
* @args: The arguments passed to the pwd command (ignored)
* @input: The input string (ignored)
*
* Return: 0 on success, 1 on failure
*/
...
int builtin_pwd(shell_context_t *ctx, char **args, char *input)
{
	...
	if (!cwd)
	{
		fprintf(stderr, "%s: 1: pwd: failed to get current working directory\n",
			ctx->program_name);
		return (1);
	}
	printf("%s\n", cwd);
